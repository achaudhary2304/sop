...
# Compute Laplacian
laplacian_index, laplacian_weight = get_laplacian(edge_index, normalization='sym')

# Features and Labels
features = torch.tensor(X, dtype=torch.float32)
labels = torch.tensor(y, dtype=torch.long)

# 4️⃣ GCN Model with Laplacian Propagation
class TrustGCN(nn.Module):
    ...
    def forward(self, x, laplacian_index, laplacian_weight):
        """
        Propagation using Laplacian matrix in GCN layers.
        """
        x = self.conv1(x, laplacian_index, laplacian_weight)
        x = F.elu(x)
        x = self.conv2(x, laplacian_index, laplacian_weight)
        x = F.elu(x)
        x = self.conv3(x, laplacian_index, laplacian_weight)
        x = F.elu(x)
        x = self.conv4(x, laplacian_index, laplacian_weight)
...
# 5️⃣ Training with SIR Loss and Laplacian Propagation
for epoch in range(200):
    model.train()
    optimizer.zero_grad()
    
    # Use the Laplacian matrix in propagation
    output = model(features, laplacian_index, laplacian_weight)
...
with torch.no_grad():
    model.eval()
    log_probs = model(features, laplacian_index, laplacian_weight)
